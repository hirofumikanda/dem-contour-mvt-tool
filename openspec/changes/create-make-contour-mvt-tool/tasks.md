...
- [x] 4.1 mapshaperを用いて、100m間隔ndjsonに対しWeighted Visvalingam簡略化（`-simplify weighted ...`）を適用するラッパー処理を実装する
- [x] 4.2 500m間隔ndjsonに対しても同様にmapshaperでの簡略化を適用する（100mより強めの簡略化パラメータを設定する）
- [x] 4.3 Chaikinのコーナーカット平滑化を行うPythonスクリプトを実装する（ndjsonを1行ずつ読み、LineString/MultiLineStringの座標配列に指定反復回数のコーナーカットを適用し、始点・終点を保持して書き戻す）
- [x] 4.4 簡略化（mapshaper）→平滑化（Chaikinスクリプト）の順で100m・500mのndjsonに適用するパイプライン処理を実装する
- [x] 4.5 10m間隔のndjsonにはこのステップを適用しない（未加工のまま次段へ渡す）ことをスクリプト上で明示する
- [x] 4.6 簡略化・平滑化前後で頂点数が減少していること、および形状が過度に歪んでいないこと（自己交差が発生していないこと）を確認する検証処理を実装する
...
- [x] 7.1 1〜6の各ステップをエントリーポイントスクリプトから順に実行できるように結線する（各ステップの入出力ファイルパスを一貫させる）
- [x] 7.2 主要パラメータ（等高線間隔、ズーム範囲、簡略化強度、Chaikin反復回数、入出力ディレクトリ）を引数または設定ファイルで指定できるようにする
- [x] 7.3 `tif/`配下のサンプルGeoTIFF（2ファイル）を入力にエントリーポイントスクリプトを実行し、エラーなくPMTilesが生成されることを確認する

## 8. テスト (Issue: #8)

- [x] 8.1 サンプルGeoTIFFを使ったエンドツーエンドテスト（VRT生成〜PMTiles生成まで一気通貫）を作成し、各中間生成物と最終PMTilesの存在・妥当性（ズーム範囲、タイル数、ジオメトリの妥当性など実際の内容）を検証する
- [x] 8.2 簡略化・平滑化処理単体のテストを作成し、意図的に角張った入力ラインに対して頂点数が減ること・角度が緩和されること・始終点が保持されることを具体的な座標値で検証する（無意味な `true === true` 等のアサーションは書かない）
- [x] 8.3 10m間隔のデータが簡略化・平滑化をスキップし元データと一致することを検証するテストを作成する
- [x] 8.4 異常系（NoDataのみのGeoTIFF、範囲が重ならない複数GeoTIFF、空の等高線結果など）に対してパイプラインが適切にエラーまたは空成果物を扱うことを確認するテストを作成する

## 9. ドキュメント (Issue: #9)

- [x] 9.1 パイプラインの使い方（前提ツール、実行コマンド、パラメータ、出力物の説明）をREADME等にまとめる
- [x] 9.2 デフォルトパラメータ（簡略化強度、Chaikin反復回数）の決定理由をコメントまたはドキュメントに残す
